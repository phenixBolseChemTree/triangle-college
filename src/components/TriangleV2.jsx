...
const Triangle = () => {
    ...
    const handleSubmit = (e) => {
        ...
        // if (per === NaN) {
        //     setPerimetr('Вы забыли написать число');
        // }
        setPerimetr(per);
    ...
    return (
        ...
        {/* {!isNaN(perimeter) && <p className="text-[25px] mb-3">Периметр треугольника равен =  <span className="text-red-400 text-[40px]">{perimeter}</span></p>} */}
            ...
            <div className="zone-1 flex w-[500px] h-[500px] items-center justify-center">
                <form className="w-[278px]" onSubmit={handleSubmit} action="">
                    <div className="flex flex-col mb-5">
                        ...
                        <input required='true' className="rounded-xl p-2 bg-gray-600" value={leftFormA} onChange={(e) => setLeftFormA(e.target.value)} type="number" />
                        <p className="mt-2">Право</p>
                        <input required='true' className="rounded-xl p-2 bg-gray-600" value={rightFormB} onChange={(e) => setRightFormB(e.target.value)} type="number" />
                        <p className="mt-2">Нижний</p>
                        <input required='true' className="rounded-xl p-2 bg-gray-600" value={bottomFormC} onChange={(e) => setBottomFormC(e.target.value)} type="number" />
                    </div>
